...
//function UpdateStatus(id, post, url) {
//    $.post(sitename + url, post, function (data) {
//        var ArrData = data.split("*****");
//        if (ArrData[1] == "1") {
//            AlertAction("Cập nhật thành công!");
//            $("#td_" + id).show().html(ArrData[2]);
//            $("#tdedit_" + id).show().html(ArrData[3]);
...
function UpdateStatus(id, post, url) {
    $.post(sitename + url, post, function (data) {
        ...
        if (data == 1) {                     
            $("#tr_" + id).hide().remove();
            AlertAction("Đã xử lý đăng website!");
...
function GuiDuyet(id, post, url) {
    if (confirm("Bạn muốn gửi bài viết này cho quản lý duyệt?")) {
        $.post(sitename + url, post, function (data) {
            if (data == 1) {
                AlertAction("Đã gửi duyệt thành công!");
                $("#tr_" + id).hide().remove();
            } else {
                alert(data);
            }
...
function HuyGuiDuyet(id, post, url) {
    if (confirm("Bạn muốn hủy gửi duyệt bài viết này để chỉnh sửa?")) {
        $.post(sitename + url, post, function (data) {
            if (data == 1) {
                AlertAction("Đã hủy gửi duyệt thành công!");
                $("#tr_" + id).hide().remove();
            } else {
                alert(data);
            }
...
function Duyet(id, post, url) {
    if (confirm("Bạn muốn duyệt bài viết này?")) {
        $.post(sitename + url, post, function (data) {
            if (data == 1) {
                AlertAction("Đã duyệt thành công!");
                $("#tr_" + id).hide().remove();
            } else {
                alert(data);
            }
...
function BoDuyet(id, post, url) {
    if (confirm("Bài viết này chưa đạt yêu cầu?")) {
        $.post(sitename + url, post, function (data) {
            if (data == 1) {
                AlertAction("Đã bỏ duyệt thành công!");
                $("#tr_" + id).hide().remove();
            } else {
                alert(data);
            }
